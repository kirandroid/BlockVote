...
final BehaviorSubject<File> _electionCoverController = BehaviorSubject<File>();
Function(File) get _electionCoverCaptured => _electionCoverController.sink.add;
Stream<File> get __electionCoverFileStream => _electionCoverController.stream;

class ElectionCoverImagePicker extends StatefulWidget {
  ...
  @override
  ElectionCoverImagePickerState createState() =>
      ElectionCoverImagePickerState();
}

class ElectionCoverImagePickerState extends State<ElectionCoverImagePicker> {
  //

  @override
  void initState() {
    super.initState();
    __electionCoverFileStream.listen((File file) {
    ...
    if (url != null) {
      DefaultCacheManager().downloadFile(url).then((var info) {
        _electionCoverCaptured(info.file);
      });
    }
  }
...
class RectangularImageHolder extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<File>(
      stream: __electionCoverFileStream,
